...
6. **BACKLOG.md is read-only (Linear is source of truth).** `docs/BACKLOG.md` is auto-generated by the linear-sync workflow. Never edit BACKLOG.md directly — all backlog management (creating issues, updating priorities, changing statuses) happens in Linear. The daily linear-sync workflow pushes the rendered state to BACKLOG.md. If BACKLOG.md appears out of date, trigger the linear-sync workflow manually rather than editing the file.
